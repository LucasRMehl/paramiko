...
class Channel (ClosingContextManager):
    ...
    def recv_exit_status(self):
        ...
        .. warning::
            ...
            sufficiently large `.BufferedPipe.read` (or if there are no threads
            calling `.BufferedPipe.read` in the background).

            In these cases, ensuring that `.recv_exit_status` is called *after*
            `.BufferedPipe.read` (or, again, using threads) can avoid the hang.
